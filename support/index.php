<?php
    ...
    $tickets = array();
    $tq = mysqli_query($conn, "SELECT ticketid,subject,status,time FROM ticketids WHERE uuid='" . $uuid . "' ORDER BY time DESC");
    ...
    for($i = 0; $i < sizeof($tickets); $i++){
        ...
        if ($tkts && mysqli_num_rows($tkts) > 0) {
            while($row = mysqli_fetch_assoc($tkts)) {
                ...
                $msg->setmessage(nl2br($row['message']));
    ...
    <body class="page-container-bg-solid page-md">
        ...
        <div class="page-container">
                                                ...
                                                <div class="portlet light ">
                                                    <div class="portlet-title">
                                                        ...
                                                        if($_SESSION['BANNED'] == true){
                                                            echo '<div class="alert alert-warning"><button class="close" data-close="alert"></button><span>Banned players are only able to submit ban appeals or update account settings on MyMCMagic.</span></div>';
                                                        }
                                                        ?>
                                                        <div class="caption caption-md">
                                                            <i class="icon-bar-chart theme-font hide"></i>
                                                            <span class="caption-subject font-blue-madison bold uppercase">Support Tickets</span>
                                                        </div>
                                                    </div>
                                                    <div class="portlet-body">
                                                        <div class="row number-stats margin-bottom-30">
                                                            ...
                                                            <div class="table-scrollable-borderless">
                                                                <div class="actions">
                                                                    <div class="btn-group btn-group-devided" data-toggle="buttons" id="new-ticket">
                                                                        <label class="btn btn-transparent blue btn-outline btn-circle btn-sm">
                                                                            <input type="radio" name="options" class="toggle" id="newTicket">Create New Ticket</label>
                                                                    </div>
                                                                </div>
                                                                <table class="table table-hover">
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        foreach($tickets as $ticket){
                                                                            ...
                                                                            echo '<tr id="ticket-' . $ticket->getid() . '"><td> ' . date('n/j/y g:i A', $ticket->gettime()) . ' </td><td style="text-overflow:ellipsis;max-width:200px;overflow:hidden;white-space:nowrap;"> ' . $ticket->getsubject() . ' </td><td style="text-overflow:ellipsis;max-width:300px;overflow:hidden;white-space:nowrap;">' . $resp . '</td><td><span class="label label-sm ' . getStatusTag($ticket->getstatus()) . '"> ' . getStatus($ticket->getstatus()) . ' </span></td><td><a href="ticket.php?id=' . $ticket->getid() . '"> View </a></td></tr>';
                                                                        }
                                                                            ?>
                                                                    </tbody>
                                                                </table>
            ...
            ?>
                ...
                <script>
                    document.getElementById('new-ticket').onclick = function () {
                        window.location = "new/";
                    }
                </script>
                <!-- END CORE PLUGINS -->
