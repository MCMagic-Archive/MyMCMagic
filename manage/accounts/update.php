<?php
    ...
    $resend = false;
    if(sanitize($_POST['resend']) == '1'){
        $resend = true;
    }
    $uuid = sanitize($_POST['uuid']);
    if($resend){
        if(mysqli_fetch_assoc(mysqli_query($conn, 'SELECT status FROM users WHERE uuid="' . $uuid . '";'))['status'] == 0){
            ...
            header("Location: info.php?uuid=" . $uuid . '&tag=sent');
        }else{
            header("Location: info.php?uuid=" . $uuid . '&tag=not-sent');
        }
        exit();
    }
    ...
    if($email != mysqli_fetch_assoc(mysqli_query($conn, 'SELECT email FROM users WHERE uuid="' . $uuid . '";'))['email']){
        mysqli_query($conn, "INSERT INTO activity (uuid, action, description) VALUES ('" . $uuid . "','Email Changed','" . $username . "')");
    }
    if($password != ''){
        $q .= ",password='" . $hashed . "',hash='" . $hash . "'";
        mysqli_query($conn, "INSERT INTO activity (uuid, action, description) VALUES ('" . $uuid . "','Password Changed','" . $username . "')");
    }
    if($answer != mysqli_fetch_assoc(mysqli_query($conn, 'SELECT answer_tickets FROM users WHERE uuid="' . $uuid . '";'))['answer_tickets']){
        $q .= ",answer_tickets='" . ($answer == 'on' ? '1' : '0') . "'";
    }
    mysqli_query($conn, $q . " WHERE uuid='" . $uuid . "';");
    header("Location: info.php?uuid=" . $uuid . '&tag=updated');