...
$uuid;
$username;
$pin = trim(sanitize($_POST['pin']));
$conn = mysqli_connect("mcmagicdb", "root", "cQr4qKQdkVQNFDrT");
mysqli_select_db($conn, "mymcmagic");
$res = mysqli_fetch_assoc(mysqli_query($conn, "SELECT uuid,pin FROM pins WHERE pin='" . $pin . "';"));
if($res['pin']==$pin){
    $uuid = $res['uuid'];
}else{
    header("Location: ../?r=1&tag=inv-pin");
    exit();
}
mysqli_query($conn, "DELETE FROM pins WHERE uuid='" . $uuid . "';");
mysqli_select_db($conn, "MainServer");
$ures = mysqli_fetch_assoc(mysqli_query($conn, "SELECT username FROM player_data WHERE uuid='" . $uuid . "';"));
...
mysqli_select_db($conn, "mymcmagic");
$r = mysqli_query($conn, "SELECT email FROM users WHERE email='" . $email . "';");
if (mysqli_num_rows($r) > 0) {
    header("Location: ../?tag=email-taken");
    exit();
}
mysqli_query($conn, "INSERT INTO activity (uuid, action, description) VALUES ('" . $uuid . "','Send Confirmation Email','" . $email . "')");
$hashed = sha1($password . $hash);
$qry = "INSERT INTO users (uuid, email, password, hash, dob) VALUES ('" . $uuid . "','" . $email . "','" . $hashed . "','" . $hash . "','1000-01-01 00:00:00');";
if (mysqli_query($conn, $qry)) {
    ...
    echo "Error registering! Please contact a Cast Member: " . mysqli_error($conn);
}
mysqli_close($conn);